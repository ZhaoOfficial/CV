...
# This module loads image and creates puzzle by
# dividing image into square pieces and then
# shuffles pieces to produce random puzzle.
# Note that created puzzle size may differ from
# original image size depending on given piece size.
...
def create_puzzle(args: argparse.Namespace):
    '''Creates jigsaw puzzle from input image'''
    image = cv2.imread(args.source)
    pieces, rows, columns = image_helpers.flatten_image(image, args.size)
    ...
    cv2.imwrite(args.destination, puzzle)
    print_messages(['Puzzle created with {} x {} pieces'.format(rows, columns)])
...
def parse_arguments() -> argparse.Namespace:
    ...
    description = (
        'Create puzzle pieces from input image by random shuffling.\n'
        'Maximum possible rectangle is cropped from original image.'
    )

    piece_size_help = (
        'Side of puzzle piece.\n'
        'Default: {0:2d} px\n'
        'Min:     {1:2d} px\n'
        'Max:     {2:2d} px\n'
    ).format(
        DEFAULT_PIECE_SIZE, MIN_PIECE_SIZE, MAX_PIECE_SIZE
    )
